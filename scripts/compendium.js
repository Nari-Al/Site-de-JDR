document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleDropdownSelection(selectedCategory) {
        ...
        switch (selectedCategory) {
            case 'races':
                fillCardsContentsRaces();  // Call the function to fill with race information
                break;
            case 'sorts':
                fillCardsContentsSorts();  // Call the function to fill with spell information
                break;
            case 'bestiaire':
                fillCardsContentsBestiaire();  // Call the function to fill with bestiary information
                break;
            case 'classes':
                fillCardsContentsClasses();  // Call the function to fill with class information
                break;
            default:
...
const RACES_SHEET = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTcfDbkIlX1JDGwoVEZwWewkDPW_B2sOC49u98xt7oAqOTWsjVTi-krJ7sLdCch-VfAVkbXswxtdS4Y/pub?gid=1312209995&single=true&output=csv'
const CLASSES_SHEET = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTcfDbkIlX1JDGwoVEZwWewkDPW_B2sOC49u98xt7oAqOTWsjVTi-krJ7sLdCch-VfAVkbXswxtdS4Y/pub?gid=0&single=true&output=csv'
...
async function fillCardsContentsRaces() {
    const csvData = await fetchData(SHEET = RACES_SHEET); // Fetch the CSV data
...
async function fillCardsContentsClasses() {
    const csvData = await fetchData(SHEET = CLASSES_SHEET); // Fetch the CSV data
    const characters = csvToJSON(csvData); // Convert CSV to JSON objects
    ...
    characters.forEach(character => {
        ...
        const cardSkills = document.createElement('p');
        cardSkills.classList.add('card-aptitudes');
        cardSkills.textContent = `Aptitudes: ${character['aptitudes']}`;

        const cardStat = document.createElement('p');
        cardStat.classList.add('card-stat');
        cardStat.textContent = `Statistique principale: ${character['statistiquePrincipale']}`;

        const cardSubclasses = document.createElement('p');
        cardSubclasses.classList.add('card-sous-classes');
        cardSubclasses.textContent = `Sous-classes: ${character['sousClasses']}`;
        ...
        card.appendChild(cardSubclasses);
